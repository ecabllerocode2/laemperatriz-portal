import { isValidEmail, isValidMexicanPhone, phonesMatch } from "@emperatriz/types";
import { z } from "zod";

export const portalRegistrationFieldsSchema = z
  .object({
    name: z.string().trim().min(2, "Ingresa tu nombre completo"),
    socialAlias: z
      .string()
      .trim()
      .min(2, "Ingresa tu nombre en redes sociales"),
    phone: z
      .string()
      .trim()
...
export const portalRegisterAccountSchema = portalRegistrationFieldsSchema
  .extend({
